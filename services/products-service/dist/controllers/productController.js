...
exports.getLowStock = exports.updateStock = exports.deleteProduct = exports.updateProduct = exports.createProduct = exports.getProductById = exports.getProducts = void 0;
...
const getProducts = async (req, res, next) => {
    try {
        ...
        if (storeId) {
            ...
            const result = await cacheService_1.default.getOrSet('products', cacheKey, async () => {
        ...
        logger_1.default.error('Error al obtener productos', { error: error.message });
        next(error);
    }
};
...
const getProductById = async (req, res, next) => {
    ...
    try {
        if (storeId) {
            ...
            const data = await cacheService_1.default.getOrSet('products', cacheKey, async () => {
        ...
        logger_1.default.error('Error al obtener producto', { error: error.message });
        next(error);
    }
};
...
const createProduct = async (req, res, next) => {
    ...
    if (fk_cod_cats) {
        try {
            ...
            if (!Array.isArray(pc))
                parsedCats = [Number(pc)];
            else
                parsedCats = pc.map(Number);
        }
        catch (e) {
    ...
    try {
        ...
        logger_1.default.info('Producto creado', { cod_prod: newProduct.cod_prod });
        await productRepository.createProductTienda({
            cod_prod: newProduct.cod_prod,
            storeId: 1,
            precio_venta: Number(precio_unitario),
            stock_actual: Number(stock_actual || 0),
        }).catch(err => logger_1.default.warn('No se pudo registrar en Producto_Tienda', { error: err.message }));
        await cacheService_1.default.invalidate('products');
        res.status(201).json(newProduct);
        (0, logActivity_1.default)({ user_email: req.user?.correo_usu, action: 'created', entity_type: 'product', entity_id: newProduct?.cod_prod, details: `Producto "${newProduct?.nom_prod}" creado` });
    }
    catch (error) {
        if (error.code === '23503') {
        ...
        logger_1.default.error('Error al crear producto', { error: error.message });
        next(error);
    }
};
...
const updateProduct = async (req, res, next) => {
    ...
    try {
        ...
        if (fields.precio_unitario !== undefined)
            fields.precio_unitario = Number(fields.precio_unitario);
        if (fields.stock_actual !== undefined)
            fields.stock_actual = Number(fields.stock_actual);
        if (fields.stock_minimo !== undefined)
            fields.stock_minimo = Number(fields.stock_minimo);
        if (fields.fk_cod_cats) {
            try {
                const fc = typeof fields.fk_cod_cats === 'string' ? JSON.parse(fields.fk_cod_cats) : fields.fk_cod_cats;
                if (!Array.isArray(fc))
                    fields.fk_cod_cats = [Number(fc)];
                else
                    fields.fk_cod_cats = fc.map(Number);
            }
            catch (e) {
        ...
        for (const key of Object.keys(fields)) {
            if (key === 'fk_cod_cats') {
                ...
                if (String(fields[key]) !== String(existing[key])) {
                    hasChanges = true;
                    break;
                }
            }
            else if (fields[key] !== existing[key]) {
                if (fields[key] == existing[key])
                    continue;
                hasChanges = true;
                break;
            }
        ...
        logger_1.default.info('Producto actualizado', { cod_prod: id });
        await cacheService_1.default.invalidate('products');
        res.status(200).json(result.rows[0]);
        (0, logActivity_1.default)({ user_email: req.user?.correo_usu, action: 'updated', entity_type: 'product', entity_id: String(id), details: `Producto "${result.rows[0]?.nom_prod}" actualizado` });
    }
    catch (error) {
        if (error.code === '23503') {
            res.status(400).json({ error: `Una de las categorías proporcionadas no existe.`, code: 'FK_NOT_FOUND' });
        ...
        logger_1.default.error('Error al actualizar producto', { error: error.message });
        next(error);
    }
};
...
const deleteProduct = async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            logger_1.default.warn('No se pudo contactar a orders-service para verificar ventas', { error: fetchErr.message });
        }
        ...
        logger_1.default.info('Producto eliminado', { cod_prod: id });
        await cacheService_1.default.invalidate('products');
        res.status(200).json({ message: 'Producto eliminado exitosamente.' });
        (0, logActivity_1.default)({ user_email: req.user?.correo_usu, action: 'deleted', entity_type: 'product', entity_id: String(id), details: `Producto "${result.rows[0]?.nom_prod}" eliminado` });
    }
    catch (error) {
        logger_1.default.error('Error al eliminar producto', { error: error.message });
        next(error);
    }
};
...
const updateStock = async (req, res, next) => {
    ...
    try {
        ...
        logger_1.default.info('Stock actualizado', { cod_prod: id, stock_actual: producto.stock_actual });
        await cacheService_1.default.invalidate('products');
        const alertaStockCritico = producto.stock_actual <= producto.stock_minimo;
        if (alertaStockCritico) {
            logger_1.default.warn('ALERTA: Stock crítico', {
                cod_prod: id, stock_actual: producto.stock_actual, stock_minimo: producto.stock_minimo,
            });
            const redisClient = cacheService_1.default.getRedis();
            if (redisClient) {
        ...
        logger_1.default.error('Error al actualizar stock', { error: error.message });
        next(error);
    }
};
...
const getLowStock = async (req, res, next) => {
    try {
        ...
        logger_1.default.error('Error al obtener productos con bajo stock', { error: error.message });
        next(error);
    }
};
