...
async function authenticate() {
    const url = `${env_js_1.default.factus.apiUrl}/oauth/token`;
    const body = new URLSearchParams({
        grant_type: 'password',
        client_id: env_js_1.default.factus.clientId,
        client_secret: env_js_1.default.factus.clientSecret,
        username: env_js_1.default.factus.username,
        password: env_js_1.default.factus.password,
    });
...
async function getAccessToken() {
    ...
    if (!env_js_1.default.factus.configured) {
    ...
    try {
        ...
        if (refreshToken) {
            try {
                ...
                logger_js_1.default.info('Factus token renovado via refresh_token');
                return accessToken;
            }
            catch (refreshErr) {
                logger_js_1.default.warn('Refresh token fallo, haciendo login completo', { error: refreshErr.message });
                refreshToken = null;
            }
        }
        ...
        tokenExpiresAt = Date.now() + (data.expires_in || 600) * 1000 - 30000;
        logger_js_1.default.info('Factus login exitoso', { expiresIn: data.expires_in });
        return accessToken;
    }
    catch (err) {
...
async function createInvoice(order) {
    if (!env_js_1.default.factus.configured) {
        logger_js_1.default.warn('Factus no configurado — simulando factura electronica');
        return generateMockInvoice(order);
    }
    try {
        const token = await getAccessToken();
        const payload = buildInvoicePayload(order);
        const url = `${env_js_1.default.factus.apiUrl}/v2/bills/validate`;
        const res = await fetch(url, {
            method: 'POST',
            headers: {
        ...
        if (!res.ok) {
            const errBody = await res.text();
            logger_js_1.default.warn('Factus rechazo la factura, usando simulacion', { status: res.status, error: errBody });
            const mockResult = generateMockInvoice(order);
            mockResult.factus_error = `${res.status}: ${errBody}`;
            return mockResult;
        }
        const data = await res.json();
        logger_js_1.default.info('Factura electronica emitida via Factus', { orderId: order.id_vent, number: data.data?.number });
        ...
        const cufe = crypto_1.default.createHash('sha384').update(`FACTUS-${bill.number}-${Date.now()}`).digest('hex');
        ...
        logger_js_1.default.warn('Error en Factus, usando simulacion', { error: err.message });
        const mockResult = generateMockInvoice(order);
...
function generateMockInvoice(order) {
    ...
    const cufe = crypto_1.default.createHash('sha384').update(`KIORA-VENTA-${order.id_vent}-${mockFecha}-${Date.now()}`).digest('hex');
...
async function deleteInvoice(referenceCode) {
    if (!env_js_1.default.factus.configured) {
        logger_js_1.default.warn('Factus no configurado — omitiendo anulacion fiscal');
        return { status: 'SKIPPED', message: 'Factus no configurado' };
    }
    try {
        const token = await getAccessToken();
        const url = `${env_js_1.default.factus.apiUrl}/v2/bills/destroy/reference/${referenceCode}`;
        const res = await fetch(url, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`,
        ...
        if (!res.ok) {
            ...
            logger_js_1.default.warn('Factus no pudo anular la factura', { referenceCode, status: res.status, error: errBody });
            return { status: 'FAILED', factus_error: `${res.status}: ${errBody}` };
        }
        const data = await res.json();
        logger_js_1.default.info('Factura anulada en Factus', { referenceCode });
        return { status: 'DELETED', factus_response: data };
    }
    catch (err) {
        logger_js_1.default.warn('Error anulando factura en Factus', { referenceCode, error: err.message });
