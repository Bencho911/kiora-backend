...
exports.generateSalesExcel = void 0;
...
const generateSalesExcel = async (data, stream) => {
    ...
    (data.dataset || []).forEach((row) => {
        if (!row.detalle_id)
            return; // Skip ventas sin líneas de detalle
    ...
    facturasMap.forEach((factura) => {
        ...
        if (factura.factura_emitida_en)
            addedRow.getCell('factura_emitida_en').numFmt = 'DD/MM/YYYY HH:mm';
    });
    ...
    (data.ventas_por_dia || []).forEach((row) => {
        ...
        if (row.fecha)
            addedRow.getCell('fecha').numFmt = 'DD/MM/YYYY';
    });
