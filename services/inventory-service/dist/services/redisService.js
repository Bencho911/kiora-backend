...
const redisClient = process.env.REDIS_HOST
    ...
    : null;
if (redisClient) {
    redisClient.on('connect', () => logger_1.default.info('Conectado a Redis para Sistema de Reservas SAGA'));
    redisClient.on('error', (err) => logger_1.default.error('Error conectando a Redis', { error: err.message }));
}
const lockInventory = async (orderId, items, ttlSeconds = 600) => {
    if (!redisClient)
        throw new Error('Redis no configurado, modo degradado.');
...
const getAdminEmails = async () => {
    ...
    try {
        ...
        logger_1.default.warn('No se pudieron obtener correos de administradores (Redis)', { error: err.message });
        return [];
    }
};
/**
...
const emitLowStockAlert = async (productData) => {
    ...
    try {
        ...
        logger_1.default.info('Notificación de stock bajo enviada a Stream', { cod_prod: productData.cod_prod });
    }
    catch (e) {
