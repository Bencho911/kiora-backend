...
async function createOrder(data) {
    ...
    logger_1.default.info('Venta creada', { id_vent: order.id_vent, store_id: order.store_id });
    return order;
}
...
async function completeOrder(orderId, reqHeaders, stripePaymentId) {
    ...
    if (order.estado === 'completada') {
        logger_1.default.warn('Outbox: orden ya completada, ignorando (idempotente)', { orderId });
    ...
    try {
        ...
        if (stripePaymentId) {
            await client.query('UPDATE Ventas SET stripe_payment_id = $1, metodopago_usu = $2 WHERE id_vent = $3', [stripePaymentId, 'stripe_tarjeta', orderId]);
        }
        // 2. Generar factura
        ...
        for (const item of order.items) {
            await orderRepository.insertOutboxEvent('inventory.movement', {
                tipo_mov: 'salida',
        ...
        await orderRepository.insertOutboxEvent('factus.invoice', {
            orderId: Number(orderId),
            montofinal_vent: updatedOrder.montofinal_vent,
            metodopago_usu: updatedOrder.metodopago_usu,
        }, client);
        await client.query('COMMIT');
        logger_1.default.info('Transacción Outbox completada', {
            id_vent: orderId,
            estado: 'completada',
        ...
        logger_1.default.error('Error en transacción Outbox de completeOrder', {
    ...
    try {
        ...
        await (0, httpClient_1.fetchWithRetry)(`${GATEWAY_URL}/api/internal/broadcast`, {
            method: 'POST',
            headers: (0, httpClient_1.outgoingHeaders)(reqHeaders),
            body: JSON.stringify({
                event: 'new_sale',
        ...
        logger_1.default.warn('No se pudo notificar nueva venta al Dashboard (WebSocket)', { error: wsErr.message });
    }
    logger_1.default.info('Estado de venta consolidado', { id_vent: orderId, estado: 'completada' });
...
async function updateStatus(orderId, estado, reqHeaders) {
    ...
    if (estado === 'reembolsada') {
        logger_1.default.info('Procesando reembolso completo', { orderId });
        // 1. Reembolso en Stripe (si la venta se pagó con tarjeta)
        if (order.stripe_payment_id) {
            try {
                await stripeService.createRefund(order.stripe_payment_id);
                logger_1.default.info('Stripe: reembolso emitido', { orderId, paymentId: order.stripe_payment_id });
            }
            catch (stripeErr) {
                logger_1.default.error('CRÍTICO: Falló reembolso Stripe', {
        ...
        try {
            ...
            if (invoice && invoice.factus_invoice_number) {
                await orderRepository.insertOutboxEvent('factus.credit_note', {
                    orderId: Number(orderId),
                ...
                logger_1.default.warn('Reembolso: no se encontró factura Factus para emitir NC', { orderId });
            }
            await client.query('COMMIT');
            logger_1.default.info('Reembolso encolado vía Outbox (inventario + NC Factus)', { orderId, items: order.items.length });
            return { ok: true, data: updated.rows[0] };
        }
        catch (err) {
            await client.query('ROLLBACK');
            logger_1.default.error('Error en transacción de reembolso Outbox', { orderId, error: err.message });
            throw err;
        }
        finally {
            client.release();
    ...
    logger_1.default.info('Estado de venta actualizado', { id_vent: orderId, estado });
    return { ok: true, data: result.rows[0] };
}
