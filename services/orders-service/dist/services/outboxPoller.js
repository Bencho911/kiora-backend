...
const stripeService = __importStar(require("./stripeService"));
const factusService = __importStar(require("./factusService"));
...
async function processEvent(event) {
    ...
    switch (event_type) {
        case 'inventory.movement': {
            const url = `${env_1.default.inventoryServiceUrl}/api/inventory/movements`;
            try {
                await (0, httpClient_1.fetchWithRetry)(url, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
                }, { maxRetries: 1, timeoutMs: httpClient_1.DEFAULT_TIMEOUT_MS });
                logger_1.default.info('Outbox: evento procesado', { id, event_type, cod_prod: payload.cod_prod });
                return { ok: true };
            }
            catch (err) {
                // ── Error de negocio (4xx): no reintentar, compensar ──
                if (err.status && err.status >= 400 && err.status < 500) {
                    logger_1.default.warn('Outbox: error de negocio en inventory', {
                ...
                logger_1.default.warn('Outbox: fallo al procesar evento', {
        ...
        case 'inventory.reserve.commit': {
            const url = `${env_1.default.inventoryServiceUrl}/api/inventory/saga/reserve/commit`;
            try {
                await (0, httpClient_1.fetchWithRetry)(url, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                logger_1.default.info('Outbox: reserva confirmada', { id, event_type, orderId: payload.orderId });
                return { ok: true };
            }
            catch (err) {
                logger_1.default.warn('Outbox: fallo al confirmar reserva', {
                    id, event_type, error: err.message, retry_count: event.retry_count,
                });
                return { ok: false, businessError: false };
            }
        ...
        case 'factus.invoice': {
            try {
                ...
                if (!order) {
                    logger_1.default.error('Outbox factus.invoice: orden no encontrada', { id, orderId });
                ...
                logger_1.default.info('Outbox: factura electrónica emitida', {
                ...
                if (err.status && err.status === 422) {
                    logger_1.default.error('Outbox factus.invoice: error de validación Factus', {
                ...
                logger_1.default.warn('Outbox factus.invoice: fallo temporal', {
        ...
        case 'factus.credit_note': {
            try {
                ...
                const order = await orderRepository.findByIdWithItems(orderId);
                if (!order) {
                    logger_1.default.error('Outbox factus.credit_note: orden no encontrada', { id, orderId });
                    return { ok: true };
                }
                // Emitir nota crédito
                ...
                logger_1.default.info('Outbox: nota crédito emitida ante DIAN', {
                ...
                if (err.status && err.status === 422) {
                    logger_1.default.error('Outbox factus.credit_note: error de validación Factus', {
                        id, errors: err.factusErrors,
                    });
                    return { ok: true }; // No reintentar
                }
                logger_1.default.warn('Outbox factus.credit_note: fallo temporal', {
                    id, error: err.message, retry_count: event.retry_count,
                });
                return { ok: false, businessError: false };
            }
        ...
        default:
            logger_1.default.warn('Outbox: tipo de evento desconocido', { id, event_type });
...
async function compensateOrder(orderId, reason) {
    logger_1.default.warn('⚠️ COMPENSACIÓN AUTOMÁTICA: Iniciando', { orderId, reason });
    try {
        // 1. Obtener orden con sus datos de pago
        const orderResult = await db_1.default.query('SELECT id_vent, estado, stripe_payment_id FROM Ventas WHERE id_vent = $1', [orderId]);
        if (orderResult.rows.length === 0) {
            logger_1.default.error('Compensación: orden no encontrada', { orderId });
            return;
        }
        const order = orderResult.rows[0];
        // Evitar doble compensación
        if (order.estado === 'cancelada' || order.estado === 'reembolsada') {
            logger_1.default.warn('Compensación: orden ya compensada, ignorando', {
        ...
        if (order.stripe_payment_id) {
            try {
                ...
                logger_1.default.info('Compensación: reembolso Stripe emitido', { orderId });
            }
            catch (refundErr) {
                logger_1.default.error('CRÍTICO: Falló el reembolso Stripe en compensación', {
                    orderId,
        ...
        metrics.compensated++;
        logger_1.default.info('✅ COMPENSACIÓN AUTOMÁTICA: Completada', {
            orderId,
            newStatus,
        ...
        try {
            ...
            await (0, httpClient_1.fetchWithRetry)(`${GATEWAY_URL}/api/internal/broadcast`, {
                method: 'POST',
                headers: (0, httpClient_1.outgoingHeaders)({}),
                body: JSON.stringify({
            ...
            }, { maxRetries: 1, timeoutMs: httpClient_1.NOTIFY_TIMEOUT_MS });
        }
        catch {
            // Fire & forget
        }
        ...
        logger_1.default.error('CRÍTICO: Error en compensación automática', {
...
async function pollOnce() {
    ...
    const store = new Map([['correlationId', `poller-${crypto_1.default.randomUUID().slice(0, 8)}`]]);
    return asyncContext_1.default.run(store, async () => {
        try {
            metrics.pollCycles++;
            // Solo tomar eventos cuyo next_retry_at ya pasó
            const result = await db_1.default.query(`SELECT id, event_type, payload, retry_count, max_retries, created_at
             FROM outbox_events
            ...
            if (result.rows.length === 0)
                return;
            logger_1.default.debug(`Outbox: procesando ${result.rows.length} eventos pendientes`);
            for (const event of result.rows) {
                const { ok, businessError } = await processEvent(event);
                if (ok) {
                    // Éxito → marcar como procesado
                    metrics.processed++;
                    await db_1.default.query(`UPDATE outbox_events
                    ...
                    await db_1.default.query(`UPDATE outbox_events
                     ...
                     WHERE id = $1`, [event.id, 'Business error: inventory rejected the operation']);
                    ...
                    if (orderId) {
                        ...
                        logger_1.default.error('Outbox: no se pudo extraer orderId para compensar', {
                    ...
                    if (newRetryCount >= event.max_retries) {
                        ...
                        await db_1.default.query(`UPDATE outbox_events
                         ...
                         WHERE id = $1`, [event.id, newRetryCount]);
                        logger_1.default.error('Outbox: evento enviado a DLQ (max retries)', {
                            id: event.id,
                        ...
                        await db_1.default.query(`UPDATE outbox_events
                         SET retry_count = $2,
                             ...
                             last_error = $4
                         WHERE id = $1`, [event.id, newRetryCount, Math.round(backoffMs), 'Retry scheduled']);
                        logger_1.default.info('Outbox: evento reprogramado con backoff', {
...
function startPoller() {
    logger_1.default.info('Outbox poller iniciado', { intervalMs: POLL_INTERVAL_MS, batchSize: BATCH_SIZE });
    const intervalId = setInterval(pollOnce, POLL_INTERVAL_MS);
    return {
        stop: () => {
            clearInterval(intervalId);
            logger_1.default.info('Outbox poller detenido');
        },
    };
}
