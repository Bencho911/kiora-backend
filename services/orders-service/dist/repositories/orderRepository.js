...
exports.checkProductInSales = exports.getStats = exports.remove = exports.updatePaymentInfo = exports.updateStatus = exports.insertOutboxEvent = exports.createWithItems = exports.findByIdWithItems = exports.findById = exports.countAll = exports.findAll = void 0;
 ...
 */
/* ── Ventas ──────────────────────────────────────────────────────────────── */
const findAll = ({ limit = 20, offset = 0, store_id = null } = {}) => {
    if (store_id) {
        return db_1.default.query(`SELECT v.*, 
             ...
             LIMIT $2 OFFSET $3`, [store_id, limit, offset]);
    }
    return db_1.default.query(`SELECT v.*, 
            (SELECT string_agg(COALESCE(nom_prod, 'Prod #' || cod_prod), ', ') FROM Producto_Venta pv WHERE pv.fk_id_vent = v.id_vent) as productos_resumen
         FROM Ventas v 
...
const countAll = () => db_1.default.query('SELECT COUNT(*) FROM Ventas');
exports.countAll = countAll;
const findById = (id_vent) => db_1.default.query('SELECT * FROM Ventas WHERE id_vent = $1', [id_vent]);
...
const findByIdWithItems = async (id_vent) => {
    const [venta, items] = await Promise.all([
        db_1.default.query('SELECT * FROM Ventas WHERE id_vent = $1', [id_vent]),
        db_1.default.query('SELECT * FROM Producto_Venta WHERE fk_id_vent = $1 ORDER BY id', [id_vent]),
    ]);
    if (venta.rows.length === 0)
        return null;
    return { ...venta.rows[0], items: items.rows };
};
exports.findByIdWithItems = findByIdWithItems;
/**
 ...
 */
const createWithItems = async ({ metodopago_usu, items, descuento_global, store_id = 1, tipo_entrega = 'PICKUP', fk_id_mesa = null }) => {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        let montofinal = items.reduce((sum, i) => sum + Number(i.precio_unit) * Number(i.cantidad), 0);
        ...
        const ventaRes = await client.query(`INSERT INTO Ventas (precio_prod_final, montofinal_vent, metodopago_usu, estado, sesion_id, store_id, tipo_entrega, fk_id_mesa)
             VALUES ($1, $2, $3, 'pendiente', $4, $5, $6, $7) RETURNING *`, [precio_prod_final, montofinal.toFixed(2), metodopago_usu || null, sesion_id, store_id, tipo_entrega, fk_id_mesa]);
        const venta = ventaRes.rows[0];
        const itemRows = [];
        for (const item of items) {
            const r = await client.query(`INSERT INTO Producto_Venta (fk_id_vent, cod_prod, cantidad, precio_unit, nom_prod)
                 VALUES ($1, $2, $3, $4, $5) RETURNING *`, [venta.id_vent, item.cod_prod, item.cantidad, item.precio_unit, item.nom_prod || null]);
...
const insertOutboxEvent = async (eventType, payload, client) => {
    const conn = client || db_1.default;
    return conn.query(`INSERT INTO outbox_events (event_type, payload) VALUES ($1, $2) RETURNING *`, [eventType, JSON.stringify(payload)]);
};
 ...
 */
const updateStatus = (id_vent, estado, client = db_1.default) => client.query('UPDATE Ventas SET estado = $1 WHERE id_vent = $2 RETURNING *', [estado, id_vent]);
 ...
 */
const updatePaymentInfo = (id_vent, paymentIntent) => db_1.default.query('UPDATE Ventas SET stripe_payment_id = $1, metodopago_usu = $2 WHERE id_vent = $3', [paymentIntent, 'stripe_tarjeta', id_vent]);
exports.updatePaymentInfo = updatePaymentInfo;
const remove = (id_vent) => db_1.default.query('DELETE FROM Ventas WHERE id_vent = $1 RETURNING id_vent', [id_vent]);
exports.remove = remove;
const getStats = async (fecha, period = '7d') => {
    const statsHoyQuery = db_1.default.query(`SELECT
         ...
         FROM Ventas
         WHERE fecha_vent::date = $1::date
           AND estado = 'completada'`, [fecha]);
    const statsAyerQuery = db_1.default.query(`SELECT
            COUNT(*)::int AS total_ventas,
            COALESCE(SUM(montofinal_vent), 0) AS monto_total,
            CASE WHEN COUNT(*) > 0 THEN SUM(montofinal_vent) / COUNT(*) ELSE 0 END AS ticket_promedio
         FROM Ventas
         WHERE fecha_vent::date = ($1::date - INTERVAL '1 day')
           AND estado = 'completada'`, [fecha]);
    const pagosHoyQuery = db_1.default.query(`SELECT 
    ...
    const evolucionQuery = db_1.default.query(evolucionQueryStr, [fecha]);
...
const checkProductInSales = async (cod_prod) => {
    const result = await db_1.default.query('SELECT 1 FROM Producto_Venta WHERE cod_prod = $1 LIMIT 1', [cod_prod]);
    return result.rows.length > 0;
};
exports.checkProductInSales = checkProductInSales;
