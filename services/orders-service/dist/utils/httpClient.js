...
exports.NOTIFY_TIMEOUT_MS = exports.DEFAULT_TIMEOUT_MS = void 0;
exports.outgoingHeaders = outgoingHeaders;
...
exports.DEFAULT_TIMEOUT_MS = 5000; // Escrituras inter-servicio
exports.NOTIFY_TIMEOUT_MS = 3000; // Notificaciones (broadcast, etc.)
/* ── Headers salientes ───────────────────────────────────────────────────── */
/**
 ...
 */
async function fetchWithRetry(url, options, { maxRetries = 3, timeoutMs = exports.DEFAULT_TIMEOUT_MS, baseDelayMs = 500, onNonRetryable, } = {}) {
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            if (res.status === 429) {
                ...
                logger_1.default.warn(`429 recibido, esperando ${waitMs}ms antes de reintentar`, { url, attempt });
            ...
            const errBody = await res.text();
            logger_1.default.warn(`Intento ${attempt}/${maxRetries}: servidor respondió ${res.status}`, {
            ...
            if (err.name === 'AbortError') {
                logger_1.default.warn(`Intento ${attempt}/${maxRetries}: timeout (${timeoutMs}ms)`, { url });
                lastError = new Error(`Timeout tras ${timeoutMs}ms: ${url}`);
                lastError.code = 'ETIMEDOUT';
            }
            else {
                // ── Error de red genérico ──
                logger_1.default.warn(`Intento ${attempt}/${maxRetries}: error de red`, {
